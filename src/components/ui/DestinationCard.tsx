...
export default function DestinationCard({ destination, tourCount = 0 }: DestinationCardProps) {
  return (
    <Link href={`/destinations/${destination.slug}`} className="group relative block aspect-[3/4] rounded-3xl overflow-hidden shadow-md hover:shadow-2xl transition-all duration-500">
      {/* Background Image */}
      <img
      ...
      <div className="absolute inset-0 bg-emerald-950/20 group-hover:bg-emerald-950/10 transition-colors duration-300" />
      ...
      <div className="absolute inset-x-0 bottom-0 p-6 flex flex-col justify-end text-white">
        <h3 className="font-serif text-xl sm:text-2xl font-bold tracking-wide leading-tight group-hover:text-emerald-300 transition-colors flex items-center justify-between">
        ...
        <div className="flex flex-wrap gap-1.5 mt-3">
          {destination.attractions.slice(0, 2).map((att) => (
            <span key={att} className="text-[10px] bg-emerald-600/30 text-emerald-200 px-2 py-0.5 rounded-md border border-emerald-500/20 font-medium">
              {att}
            </span>
          ))}
        </div>
