...
import { saveSocialPostAction, deleteSocialPostAction, uploadImageAction } from "@/app/actions";
import { Search, Plus, Trash2, X, Info, Upload, Camera, Globe, AtSign, Music2 } from "lucide-react";
import { SocialPost } from "@/services/db/types";

interface SocialFeedManagerProps {
  posts: SocialPost[];
}

const PLATFORM_ICONS: Record<SocialPost["platform"], typeof Camera> = {
  instagram: Camera,
  facebook: Globe,
  twitter: AtSign,
  tiktok: Music2,
...
export default function SocialFeedManager({ posts }: SocialFeedManagerProps) {
  const [searchQuery, setSearchQuery] = useState("");
  const [editItem, setEditItem] = useState<Partial<SocialPost> | null>(null);
  ...
  const handleOpenEdit = (p: SocialPost) => {
    setEditItem({ ...p });
    setError("");
  };

  const handleDelete = async (id: string) => {
    if (!confirm("Remove this post from the social feed?")) return;
    setLoading(true);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    const res = await saveSocialPostAction({
      ...
      post_url: editItem.post_url || undefined,
  ...
  return (
    <div className="space-y-6">
      ...
      <p className="text-[11px] text-slate-450 dark:text-slate-500 -mt-2">
        These posts are curated manually and shown as the homepage social feed.
        There is no live Instagram/Facebook API connection — add a screenshot or
        export of the real post and link back to it below.
      </p>
      ...
      {filtered.length === 0 ? (
        <div className="bg-white dark:bg-slate-900/60 border border-slate-200/40 dark:border-slate-850 rounded-2xl p-10 text-center text-xs text-slate-450 dark:text-slate-500">
          No social posts yet. Add one to feature it on the homepage.
        </div>
      ) : (
          ...
          {filtered
            ...
            .map((p) => {
              ...
              return (
                <div
                  key={p.id}
                  className="bg-white dark:bg-slate-900/60 border border-slate-200/40 dark:border-slate-850 rounded-2xl overflow-hidden shadow-sm group relative cursor-pointer"
                  onClick={() => handleOpenEdit(p)}
                >
                  <img src={p.image_url} alt={p.caption || "Social post"} className="w-full h-32 object-cover" />
                  <div className="p-3">
                    <div className="flex items-center space-x-1 text-slate-400 mb-1">
                      <Icon className="h-3 w-3" />
                      <span className="text-[9px] uppercase font-bold tracking-wider">{p.platform}</span>
                    </div>
                    {p.caption && <p className="text-[11px] text-slate-600 dark:text-slate-400 line-clamp-2">{p.caption}</p>}
                  </div>
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
        ...
        <div className="fixed inset-0 bg-slate-955/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="w-full max-w-lg bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-150 dark:border-slate-850 shadow-2xl relative max-h-[85vh] overflow-y-auto custom-scrollbar">
            ...
            <h3 className="font-serif text-lg font-bold text-slate-900 dark:text-white mb-6">
              {editItem.id ? "Edit Social Post" : "Add Social Post"}
            </h3>
            ...
            <form onSubmit={handleSave} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="flex flex-col space-y-1">
                  <label className="text-[10px] uppercase font-bold text-slate-405">Platform</label>
                  <select
                    value={editItem.platform || "instagram"}
                    onChange={(e) => setEditItem({ ...editItem, platform: e.target.value as SocialPost["platform"] })}
              ...
              <div className="flex flex-col space-y-1">
                ...
                <input
                  ...
                  placeholder="e.g. Sunset game drive in the Mara"
                  className="bg-slate-50 dark:bg-slate-950 border-none rounded-xl py-2 px-3 text-xs outline-none text-slate-800 dark:text-slate-205"
                />
              </div>

              <div className="flex flex-col space-y-1">
                <label className="text-[10px] uppercase font-bold text-slate-405">Link to Real Post (optional)</label>
                <input
                  type="url"
                  value={editItem.post_url || ""}
                  onChange={(e) => setEditItem({ ...editItem, post_url: e.target.value })}
