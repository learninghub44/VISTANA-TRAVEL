...
const statusStyles: Record<Vehicle["status"], string> = {
  Available: "text-emerald-700 dark:text-emerald-400 bg-emerald-500/10",
  Maintenance: "text-amber-700 dark:text-amber-500 bg-amber-500/10",
  Assigned: "text-sky-700 dark:text-sky-400 bg-sky-500/10",
};
          ...
          {filtered.map((v) => (
            ...
            >
              <div className="p-5 space-y-2">
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400">Driver: {v.driver_name || "Unassigned"}</p>
                <div className="flex justify-between text-xs pt-1 border-t border-slate-100 dark:border-slate-800">
                  ...
                  <span className="font-bold text-slate-600 dark:text-slate-300">{v.capacity} seats</span>
                </div>
              </div>
